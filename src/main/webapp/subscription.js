function getSubscriptionProfile() {
    ...
    var subscription_id = param.substring(param.lastIndexOf("=") + 1, param.lenght);
    $.ajax({
        type: "GET",
        url: configOptions.mainurl + "/subscription/" + subscription_id, // URL of the Perl script
        crossOrigin: true,
        async: false,
        ...
        success: function(data) {
            ...
            divContainer.innerHTML = "<h2>API Call GET /subscription/{subscriptionId}</h2><br/>";
    ...
    $.ajax({
        type: "GET",
        url: "subdata/" + subscription_id, // URL of the Perl script
        crossOrigin: true,
        async: false,
        error: function(XMLHttpRequest, textStatus, errorThrown) {
        ...
        success: function(data) {
            ...
            var divContainer = document.getElementById("showDBData");
            divContainer.innerHTML = "<h2>DB call to mt_sub</h2><br/>";
            divContainer.innerHTML += createTable(data);
        } // success
    });
...
function getSubscriptionUsers() {
    ...
    $.ajax({
        type: "GET",
        url: configOptions.mainurl + "/subscription/" + subscription_id + "/users",
        crossOrigin: true,
        async: false,
        error: function(XMLHttpRequest, textStatus, errorThrown) {
            document.body.innerHTML += "<div id=\"showAPIData\" class=\"leftdiv\"></div>";
        ...
        success: function(data) {
            ...
            console.log(data);
            console.log(Object.keys(data).length);
            document.body.innerHTML += "<div id=\"showAPIUser2Subscription\" style=\"float:left;\"></div>";
            var divContainer = document.getElementById("showAPIUser2Subscription");
            divContainer.innerHTML = "<h2>API GET Subscription Users</h2><br/>";
            divContainer.innerHTML += createTable(data);
        }
    });
}
...
function getSubscriptionGroups() {
    ...
    $.ajax({
        type: "GET",
        url: configOptions.mainurl + "/subscription/" + subscription_id + "/groups",
        crossOrigin: true,
        async: false,
        error: function(XMLHttpRequest, textStatus, errorThrown) {
            document.body.innerHTML += "<div id=\"showAPIData\" class=\"leftdiv\"></div>";
        ...
        success: function(data) {
            ...
            divContainer.innerHTML = "<h2>API GET Subscription Groups</h2><br/>";
            divContainer.innerHTML += createTable(data);
        }
    });
}

function getSubscriptionMemberships() {
    removedivs();
    var param = window.location.search.substring(1);
    var subscription_id = param.substring(param.lastIndexOf("=") + 1, param.lenght);
    $.ajax({
        type: "GET",
        url: configOptions.mainurl + "/subscription/" + subscription_id + "/memberships",
        crossOrigin: true,
        async: false,
        error: function(XMLHttpRequest, textStatus, errorThrown) {
           document.body.innerHTML += "<div id=\"showAPIData\" class=\"leftdiv\"></div>";
...
function createTable(datax) {
    ...
    for (var i = 0; i < keys.length; i++) {
        ...
        if (typeof datax[keys[i]] != 'object' || datax[keys[i]] == null) {
            ...
            //            if(keys[i] == "id" && currrentAttribute == "user2SubscriptionAssociations"){
